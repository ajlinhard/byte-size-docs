...
- At the end of each week (or first thing Monday) we will attempt to rebase Sub-team branches, only to ignoring this for specific approved reasons.
...
The biggest rule: **merge at least every 2 days**. If a branch lives longer than 3-4 days, it's accumulating conflict debt. Encourage your sub-team leads to check for stale branches weekly. GitLab's branch list makes this easy to audit.
...
- **Pipelines must pass** before merge — no exceptions. Make your CI fast (under 10 minutes ideally) so this doesn't become a bottleneck people want to skip.
- **Delete branches on merge** automatically (GitLab has a setting for this).
- Keep MRs small. A good rule of thumb is under 400 lines changed. Big MRs get rubber-stamped; small ones get real review.
...
With 3-4 sub-teams, the biggest risk is teams stepping on each other. A few things help:

- Have each sub-team lead do a quick daily check of open MRs — stale MRs are a sign of a blocked teammate.
...
- Schedule a weekly 15-minute sync just to surface cross-team dependencies before they become merge conflicts.
